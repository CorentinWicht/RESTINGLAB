...
% History
% 03/26/2019 Makoto and Chiyuan. Supported 'availableRAM_GB'. GUI switched to GUIDE-made. Rank check implemented.
% 09/04/2012 Christian. Created.
%
...
if ~strcmp(flatline_crit,'off')
    EEG = clean_flatlines(EEG, flatline_crit, max_allowed_jitter); end

% high-pass filter the data
if ~strcmp(highpass_band,'off')
    EEG = clean_drifts(EEG, highpass_band, attenuation); end
if nargout > 1
    HP = EEG; end
...
if ~strcmp(chancorr_crit,'off') || ~strcmp(line_crit,'off') %#ok<NODEF>
    ...
    if strcmp(line_crit,'off')
        line_crit = 100; end    
    try 
        EEG = clean_channels(EEG, chancorr_crit, line_crit, clchan_window_len, channel_crit_maxbad_time, num_samples, subset_size); 
    catch e
        if strcmp(e.identifier,'clean_channels:bad_chanlocs')
            ...
            EEG = clean_channels_nolocs(EEG, nolocs_channel_crit, nolocs_channel_crit_excluded, clchan_window_len, channel_crit_maxbad_time, linenoise_aware); 
        else
            rethrow(e);
        end
    end
...
if dataRank == EEG.nbchan
    disp('Rank check passed.')
elseif dataRank < EEG.nbchan
    error('This data is rank deficient. Make the data full-ranked.')
end



% repair bursts by ASR (Updated by Makoto. 03/25/2019)
if ~strcmp(burst_crit,'off')
    EEG = clean_asr(EEG, burst_crit, asr_windowlen, asr_stepsize, maxdims, burst_crit_refmaxbadchns, burst_crit_reftolerances, ref_wndlen, usegpu, 'availableRAM_GB', availableRAM_GB); end
if nargout > 2
    BUR = EEG; end
...
if ~strcmp(window_crit,'off') && ~strcmp(window_crit_tolerances,'off')
    ...
    EEG = clean_windows(EEG, window_crit, window_crit_tolerances, clwin_window_len, window_overlap, max_dropout_fraction, min_clean_fraction, truncate_quant, clwin_step_sizes, shape_range); 
end
