        ...
        function UpdateField(self, name)
            ...
            if field.CanAccept(value)
                try
                    ...
                    if ~isequal(field.PropertyData.Value,value) && ~isempty(field.PropertyData.ChildrenSets)
                        curRow = self.Table.getSelectedRow();
                        javaMethodEDT('expandRow',self.Table,curRow,false); % self.Table.expandRow(self.Table.getSelectedRow(),false);
                        % value changed for a property with alternative chilren sets: relink!
                        field.Value = value;
                        javaMethodEDT('expandRow',self.Table,curRow,true); % self.Table.expandRow(self.Table.getSelectedRow(),true);
                    end
