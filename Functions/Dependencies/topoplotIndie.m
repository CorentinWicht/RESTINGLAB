...
if nargs > 2
    for i = 1:2:length(varargin)
        ...
        switch Param
            ...
            case 'shading'
                SHADING = lower(Value);
                if ~any(strcmp(SHADING,{'flat','interp','w'}))
...
% WHY WOULD YOU REMOVE ELECTRODES OUTSIDE THE PLOTTING CIRCLE ??
...
elseif strcmp(SHADING,'w')
    handle = surface(Xi*unsh,Yi*unsh,zeros(size(Zi)),Zi,'EdgeColor','none','FaceColor',[1 1 1]);
else
...
if strcmp(SHADING,'interp') || strcmp(SHADING,'w')
...
if strcmp(ELECTRODES,'on')   % plot electrodes as spots
    ...
    % NEW
    % Plot red X instead of black . for significant electrodes
    %if SigElectrodes~=0
    if ~isempty(SigElectrodes)
        % One colour for all electrodes
        if ~iscell(SigElectrodes)
            hold on
            hp2 = plot3(y(SigElectrodes),x(SigElectrodes),ones(size(x(SigElectrodes))),...
                'X','Color',ColSigElect,'markersize',10,'linewidth',2,'hittest','off');
            hold off
            
        % More than one colour
        else
            for m=1:size(SigElectrodes,1)
                hold on
                hp2 = plot3(y(SigElectrodes{m,:}),x(SigElectrodes{m,:}),ones(size(x(SigElectrodes{m,:}))),...
                    'X','Color',ColSigElect{m},'markersize',10,'linewidth',2,'hittest','off');
                hold off
            end
