...
%   StatsType  = [String] 'd', dependent or, 'i', independent samples.
%   (def='d')
...
StatsType   = 'd'; % default sample type
...
% Computing neighbouring channels (only if not provided as input)
if isempty(ChN)
...
try
    ...
    Results = ept_TFCE(Data{1},Data{2}, EEG.chanlocs,'nPerm', N_Permutes,...
        'rSample', EEG.srate,'ChN', ChN,'flag_tfce',TFCE,'type',...
        StatsType,'e_h',E_H);
catch
