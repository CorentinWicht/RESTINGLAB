...
%domainLocation = headGrid.insideBrainCube.*membershipCube;
...
distanceFromDomainLocationsToAllCSFLocations = pdist2_fast(domainLocation, csfVertices);
...
for i=1:size(distanceFromDomainLocationsToAllCSFLocations, 1)
    ...
    if distanceToCSF < distanceToKeepFromCSF
        ...
        triangleDistanceToDomainLocation = pdist2_fast(triangleCenter, domainLocation(i,:));
...
distance = pdist2_fast(fineGridPosition, domainLocation);
...
if inputOptions.showProjectedOnMrs
    ...
    distanceFromReducedToOriginalVertices = pdist2_fast(single(reducedIsosurfacePatch.vertices), isosurfaceFacesAndVerices.vertices);
