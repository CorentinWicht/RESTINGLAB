...
if strcmpi(FreqBandsAnalyses,'yes')
    ...
    % Trying to build a linear neighbouring matrix based on freq bins
    % IMPOSSIBLE !!! NEED TO FIND SOMETHING ELSE 
    % The solution could be to bypass TFCE ?
    % If doesn't work, use without permutation (set it to 1?) and apply FDR
    % correction!!! 
    % for m=1:size(SpectFreqs,1)
    %     FreqBins(m).X = ALLEEG(1).chanlocs(m).X;
    %     FreqBins(m).Y = 1;
    %     FreqBins(m).Z = 1;
    % end
    % FreqBins.srate = ALLEEG(1).srate;
    % ChN = ept_ChN2(FreqBins, 1);
