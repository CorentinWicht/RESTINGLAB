...
if strcmpi(AnalysesSwitch{2,end},'Yes')
    ...
    for k = 2:length(AreasList)
        if ~isempty(str2num(AreasList{k})) % If user entered numbers
            ...
            load([FolderTemp '\ChanLocsBioSemi64.mat'])
            ChanList = {ChanlocsBioSemi64.labels};
...
if exist('SubjectsRemoved','var')
    Conditions_OrderCell(SubjectsRemoved,:)=[];
end

% Building the subjects selection list based on GUI data
SubjectsIncluded = [TempSubjectslist ProcessData];
FilesToProcess = 0;
...
%                         max(SpectOutputsStruc.Data(:).(FreqNames{m})); 
                ...
                % Adjusting data according to electrodes included in analysis
%                 if ~isempty(AllElectrodes)
%                     SpectOutputsStruc.Data = structfun(@(x) x(AllElectrodes),...
%                         SpectOutputsStruc.Data,'UniformOutput',0);
%                 end
                
%                 % Finding the area with max electrode-mean amplitude
%                 for m=1:size(FreqNames,1)
%                     [ClustAmpStruct.MaxClust(m,:), ClustAmpStruct.MaxIdx(m,:)] = ...
%                         max(SpectOutputsStruc.Data.(FreqNames{m}));
% %                         structfun(@(x) max(x),SpectOutputsStruc.Data,'UniformOutput',0);
%                     ClustAmpStruct.MaxIdx(m,:) = AllElectrodes(ClustAmpStruct.MaxIdx(m,:));
% 
%                     % Finding the area with min electrode-mean amplitude
%                     [ClustAmpStruct.MinClust(m,:), ClustAmpStruct.MinIdx(m,:)] = ...
%                          min(SpectOutputsStruc.Data.(FreqNames{m}));
% %                         structfun(@(x) min(x),SpectOutputsStruc.Data,'UniformOutput',0);
%                     ClustAmpStruct.MinIdx(m,:) = AllElectrodes(ClustAmpStruct.MinIdx(m,:));
%                 end
